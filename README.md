...
3. Copy the generated public key value and add as an API key under the OCI user on OCI portal
4. In the codespace, execute `init-local-oci.sh` again to further setup
    - Setup kubeconfig (the setup may fail due to the API key not yet propagated across OCI; retry should work)
